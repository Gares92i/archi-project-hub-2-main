...
export const AnnotationReserveItem: React.FC<AnnotationReserveItemProps> = ({
  ...
  return (
    <TableRow>
      // Dans la première cellule du tableau, ajoutez l'ID d'annotation
<TableCell>
  {reserve.annotationId ? 
    `#${reserve.annotationId.substring(reserve.annotationId.length - 4)}` : 
    "-"}
</TableCell>
        <Select
          value={reserve.lot}
      ...
      <TableCell>
        <Input
          ...
          onChange={(e) =>
            updateReserve(reserve.id, { location: e.target.value })
          }
      ...
      <TableCell>
        <Input
          ...
          onChange={(e) =>
            updateReserve(reserve.id, { description: e.target.value })
          }
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto p-0">
            <Calendar
              mode="single"
              selected={
                reserve.createdAt ? new Date(reserve.createdAt) : undefined
              }
              onSelect={(date) =>
                date &&
                updateReserve(reserve.id, { createdAt: date.toISOString() })
              }
              initialFocus
            />
          </PopoverContent>
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto p-0">
            <Calendar
              mode="single"
              selected={
                reserve.resolvedAt ? new Date(reserve.resolvedAt) : undefined
              }
              onSelect={(date) => {
                if (date) {
