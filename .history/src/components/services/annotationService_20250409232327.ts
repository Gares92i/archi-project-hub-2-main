...
export const getProjectAnnotations = async (projectId: string): Promise<Annotation[]> => {
  try {
    ...
    // Enrichir les annotations avec les informations du document
    const allAnnotations = documents.flatMap(doc =>
  doc.annotations?.map(ann => ({
    ...ann,
    documentId: doc.id,
    documentName: doc.name,
    // Si doc.url est null ou undefined, utilisez une URL de placeholder
    documentUrl: doc.url || `/images/document-placeholder-${doc.id}.jpg`
  })) || []
);
