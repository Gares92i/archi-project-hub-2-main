...
export const PlanViewer: React.FC<PlanViewerProps> = ({
  ...
  const initCanvas = () => {
    ...
    try {
      ...
      if (document.url && document.url !== '/placeholder.svg') {
        ...
        img.onerror = (error) => {
          console.error("Erreur de chargement de l'image:", document.url, error);
  ...
  // Ajouter cette fonction pour gérer le téléchargement de document
  const handleDocumentUpload = useCallback((url: string, filename: string) => {
    setTempDocumentData({ url, filename });
    setIsDocumentDialogOpen(true);
  }, []);

  // Ajouter cette fonction pour finaliser la mise à jour du document avec le nom choisi
