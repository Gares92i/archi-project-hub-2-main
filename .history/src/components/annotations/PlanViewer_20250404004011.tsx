...
export const PlanViewer: React.FC<PlanViewerProps> = ({
  ...
  const addAnnotationsToCanvas = useCallback(() => {
    ...
    annotations.forEach((annotation) => {
      // S'assurer que l'annotation a une position valide (soit via x/y, soit via position)
      if (
        !annotation ||
      ...
      const circle = new fabric.Circle({
        ...
        top: 0,
        radius: 10,
        fill: annotation.resolved // Utiliser resolved au lieu de isResolved
          ? "rgba(0, 200, 0, 0.5)"
          : "rgba(255, 0, 0, 0.5)",
        stroke: annotation.resolved ? "green" : "red", // Ici aussi
        strokeWidth: 2,
      });
      ...
      const group = new fabric.Group([circle, label], {
        left: posX - 10, // Utiliser posX au lieu de annotation.position.x
        top: posY - 10,  // Utiliser posY au lieu de annotation.position.y
        selectable: false,
        data: annotation,
      });
