...
export const AnnotationsSidebar: React.FC<AnnotationsSidebarProps> = ({
  ...
  // S'assurer que annotations est toujours un tableau
  const safeAnnotations = annotations || [];
  
  // Calculer les statistiques seulement si nous avons des annotations
  const resolvedCount = safeAnnotations.filter((ann) => ann.resolved).length;
