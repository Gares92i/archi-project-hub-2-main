...
export const PlanViewer: React.FC<PlanViewerProps> = ({
  ...
  // Modifier l'initialisation du canvas pour mieux gérer les PDF
  const initCanvas = useCallback(() => {
    ...
    console.log('Initializing canvas for document:', document);
    ...
    try {
      ...
      if (document.url && document.url !== '/placeholder.svg' && document.url.trim() !== '') {
        console.log('Loading document URL:', document.url);

        // Si c'est un PDF, traiter différemment
        if (document.type === 'pdf' || document.url.startsWith('data:application/pdf')) {
          // Cas PDF : vous devrez utiliser une bibliothèque comme pdf.js
          // pour le moment, afficher un message
          const text = new fabric.Text("Document PDF (aperçu non disponible)", {
        ...
        console.log('No valid document URL, canvas ready without image');
        setIsCanvasReady(true);
      }
    } catch (error) {
  ...
  const handleDocumentNameConfirm = useCallback(
    (name: string) => {
      if (tempDocumentData) {
        console.log('Confirming document name:', name);
  ...
  useEffect(() => {
    console.log('Document changed, reinitializing canvas:', document.id);
    initCanvas();
  ...
  }, [document.id, document.url, initCanvas]);
