  ...
  const saveProjectData = useCallback((data: Document[]) => {
    try {
      ...
      const dataToSave = data.map(doc => {
        // Pour les PDF et les images volumineuses, ne stockez pas les URLs complètes
        if (doc.type === "img" && doc.url.length > 200000) {
          return {
            ...doc,
            url: doc.url.substring(0, 100) + "...[truncated]..." // Version tronquée
          };
        }
        return doc;
      });
