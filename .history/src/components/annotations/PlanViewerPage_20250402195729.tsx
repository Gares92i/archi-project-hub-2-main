...
export const PlanViewerPage = () => {
  ...
  // Mettons à jour la fonction handleAddNewDocument avec plus de logs
  const handleAddNewDocument = (url: string, filename: string) => {
    const type = url.startsWith('data:application/pdf') ? 'pdf' : 'img';
    
    console.log('handleAddNewDocument called with:', { url: url.substring(0, 50) + '...', filename, type });
    console.log('Current activeDocument:', activeDocument?.name);
