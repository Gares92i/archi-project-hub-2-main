...
export const ConvertToTaskDialog: React.FC<ConvertToTaskDialogProps> = ({
  ...
  const [taskData, setTaskData] = useState<ConvertToTaskData>({
    ...
    projectId: projectId,
    assigneeId: "",
    dueDate: formatDateToISOString(new Date()), // Initialiser avec la date du jour
    priority: "medium",
  });

  // Mettre à jour les données quand l'annotation change
  useEffect(() => {
    if (annotation) {
      setTaskData((prev) => ({
        ...prev,
        description: annotation.comment || prev.description,
      }));
    }
  }, [annotation]);
    ...
    try {
      ...
      const taskId = await addTask({
        ...
        start: formatDateToISOString(new Date()),
          ...
          taskData.dueDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
        ),
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              name="dueDate"
              type="date"
              value={taskData.dueDate || formatDateToISOString(new Date())}
              onChange={handleChange}
            />
          </div>
