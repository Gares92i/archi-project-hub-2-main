...
// Correction de l'import dans FileUploader.tsx
...
reader.onload = (e) => {
  if (e.target?.result) {
    ...
    console.log("Fichier chargé, prêt à ouvrir le dialogue");
    setSelectedFile(file);
    setFileDataUrl(dataUrl);
    setIsNamingDialogOpen(true);
    console.log("État du dialogue après mise à jour:", { isNamingDialogOpen: true });
  }
};

// Dans handleDocumentNameConfirmed
const handleDocumentNameConfirmed = (documentName: string) => {
  console.log("Nom confirmé, données disponibles:", { 
    documentName, 
    fileDataUrl: fileDataUrl ? "présent" : "absent" 
  });
  
  if (fileDataUrl) {
    onDocumentUpdate(fileDataUrl, documentName);
    console.log("onDocumentUpdate appelé avec:", { documentName });
    // ...
  }
};
