...
import {
  ...
  Upload,
  Maximize,
  MessageSquare, // Remplacer Annotation par MessageSquare
  MoveHorizontal,
  X,
} from "lucide-react";
...
export const MainContent: React.FC<MainContentProps> = ({
  ...
  const handleClick = (e: React.MouseEvent) => {
    if (!isAddingAnnotation || !containerRef.current || !documentContainerRef.current) return;
    
    // Obtenir la position relative au conteneur du document
    ...
    const x = (e.clientX - docRect.left) / zoomLevel;
    const y = (e.clientY - docRect.top) / zoomLevel;
  ...
  const renderAnnotations = () => {
    ...
    return annotations.map((ann) => {
      ...
      // Position de base des annotations (sans transformation)
      const baseX = (ann.x || ann.position?.x || 0);
      const baseY = (ann.y || ann.position?.y || 0);
            ...
            {activeDocument && activeDocument.url && activeDocument.url !== '/placeholder.svg' ? (
              activeDocument.type === "pdf" ? (
                ...
                <div className="relative w-full h-full flex items-center justify-center">
                  ...
                  />
                  {/* Annotations positionnées en % à l'intérieur de ce conteneur */}
                  {renderAnnotations()}
                </div>
              )
            ) : (
