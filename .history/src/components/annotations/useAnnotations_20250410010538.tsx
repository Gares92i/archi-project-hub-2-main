...
export const useAnnotations = (initialDocuments: Document[] = [], projectId: string) => {
    ...
    const loadSavedData = () => {
      try {
        ...
        if (savedDocuments) {
          ...
          console.log(
            "Documents chargés depuis localStorage:",
          ...
          console.log(
            "Utilisation des documents initiaux:",
    ...
    (data: Document[]) => {
      try {
        ...
        try {
          ...
          localStorage.setItem(
            `project-annotations-${projectId}-metadata`,
            JSON.stringify(lightData)
          );
          toast.warning(
            "Stockage limité: les images des documents ne seront pas sauvegardées",
            {
              duration: 5000,
            }
          );
  ...
  const cleanupStorage = useCallback(() => {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (
          key &&
          key.startsWith("project-annotations-") &&
          key !== `project-annotations-${projectId}`
        ) {
      ...
      if (keysToCheck.length > 3) {
        ...
        for (let i = 0; i < keysToCheck.length - 3; i++) {
          ...
          console.log(
            `Nettoyage du stockage: projet ${keysToCheck[i]} supprimé`
          );
        }
      }
      ...
      localStorage.setItem(
        `project-annotations-${projectId}-lastAccess`,
        Date.now().toString()
      );
    } catch (error) {
  ...
  const updateAndSaveDocuments = useCallback(
  ...
  const handleToggleResolved = useCallback(
    (id: string, resolved?: boolean) => {
      ...
      const newResolvedState =
        resolved !== undefined ? resolved : !isCurrentlyResolved;

      const updatedAnnotations =
        activeDocument.annotations?.map((a) => {
          if (a.id === id) {
            return {
              ...a,
              resolved: newResolvedState,
              isResolved: newResolvedState, // Pour compatibilité
              resolvedDate: newResolvedState
  ...
  const handleUpdateAnnotation = useCallback(
    (id: string, data: Partial<Annotation>) => {
      ...
      if (selectedAnnotation?.id === id) {
        ...
        console.log(
          "Annotation sélectionnée mise à jour:",
  ...
  const exportAnnotations = useCallback(() => {
    try {
      ...
      link.href = url;
      link.download = `annotations-${projectId}-${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
