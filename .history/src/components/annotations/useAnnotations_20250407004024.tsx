...
export const useAnnotations = (initialDocuments: Document[], projectId: string) => {
  ...
  const handleAddAnnotation = useCallback(
    (position: { x: number; y: number }) => {
      ...
      console.log("Annotations actuelles:", updatedDocuments.find(d => d.id === activeDocument.id)?.annotations?.length || 0);

      // Mettre à jour activeDocument avec les annotations mises à jour
      const updatedActiveDocument = updatedDocuments.find(doc => doc.id === activeDocument.id);
      if (updatedActiveDocument) {
        setActiveDocument(updatedActiveDocument);
      }
    },
    [activeDocument, documents, setSelectedAnnotation, setIsAnnotationDialogOpen]
  );
