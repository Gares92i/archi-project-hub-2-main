...
export const PlanViewerPage = () => {
  ...
  const handleAddNewDocument = useCallback((url: string, filename: string) => {
    ...
    try {
      ...
      // Appeler addNewDocument sans vérifier son retour, car il est de type void
  ...
  const handleDeleteAnnotation = useCallback((annotation: Annotation) => {
    ...
    setIsAnnotationDialogOpen(false);
    setSelectedAnnotation(null); // Ajout de cette ligne pour désélectionner l'annotation
    
    // Message de confirmation
    toast.success("Annotation supprimée avec succès");
  }, [activeDocument, documents, setDocuments, setIsAnnotationDialogOpen]);
        ...
        <>
          ...
          <div className="flex-1 flex flex-col relative">
            ...
            <MainContent
              ...
              onRepositionAnnotation={(id, newPosition) => {
                if (activeDocument && handleUpdateAnnotation) {
                  handleUpdateAnnotation(id, { x: newPosition.x, y: newPosition.y });
                }
              }}
            />
          </div>
