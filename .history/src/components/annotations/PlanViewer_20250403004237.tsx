...
export const PlanViewer: React.FC<PlanViewerProps> = ({
  ...
  useEffect(() => {
    if (!fabricCanvas || !document?.url || document.url === "/placeholder.svg")
      return;
    ...
    const loadImage = () => {
      ...
      imgElement.onload = () => {
        try {
          ...
          console.error(
            "Erreur lors du chargement de l'image dans le canvas:",
            error
          );
  ...
  const handleTogglePanMode = useCallback(() => {
    ...
    toast.info(
      `Mode ${canvasMode === "select" ? "déplacement" : "annotation"} activé`
    );
