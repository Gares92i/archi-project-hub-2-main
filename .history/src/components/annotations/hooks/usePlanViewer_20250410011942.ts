...
export const usePlanViewer = ({
  ...
  // Create an adapter function for addAnnotation to match expected signature
  const adaptAddAnnotation = (annotation: Annotation, isSelected: boolean, onClick: () => void) => {
  ...
  const { setAnnotationObjects } = useAnnotationsRenderer({
    ...
    isPanning,
    onAnnotationClick,
    addAnnotation: adaptAddAnnotation
  });
