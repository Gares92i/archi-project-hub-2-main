...
export const useAnnotations = (initialDocuments: Document[], projectId: string) => {
  ...
  const handleAddAnnotation = useCallback(
    (position: { x: number; y: number }) => {
      if (!activeDocument) {
        toast.error("Veuillez sélectionner un document pour ajouter une annotation");
        return;
      }

      console.log("Ajout d'annotation à la position:", position);
      ...
      // Mettre à jour le document actif avec la nouvelle annotation
      ...
      setActiveDocument(updatedActiveDocument);
      setSelectedAnnotation(newAnnotation);
      setIsAnnotationDialogOpen(true);
      
      console.log("Documents mis à jour avec la nouvelle annotation");
      console.log("Annotations actuelles:", updatedActiveDocument.annotations?.length || 0);
    ...
    [activeDocument, documents, setDocuments, setActiveDocument, setSelectedAnnotation, setIsAnnotationDialogOpen, storageKey]
  );
  ...
  const addNewDocument = useCallback(
    (url: string, name: string, type: "pdf" | "img") => {
      console.log("addNewDocument appelé avec:", { name, type, urlPrefix: url.substring(0, 20) + "..." });
      ...
      console.log("Nouveau document créé:", newDocument.id);
      ...
      console.log("Documents après ajout:", newDocuments.length);
    },
  ...
  // Ajouter une nouvelle fonction pour mettre à jour les propriétés d'une annotation
  const handleUpdateAnnotation = useCallback(
    (annotationId: string, data: Partial<Annotation>) => {
      if (!activeDocument) return;
      ...
      setDocuments(updatedDocuments);
      setActiveDocument(updatedDocument);
      saveProjectData(updatedDocuments);
  ...
  return {
    ...
    handleUpdateAnnotation, // Nouvelle fonction
    addNewDocument,
    setDocuments,
    resetProjectData,
  };
