 ...
 */
interface CanvasCustomProperties {
  isDragging?: boolean;
  lastPosX?: number;
  lastPosY?: number;
}

interface ExtendedCanvas extends fabric.Canvas, CanvasCustomProperties {
  ...
  try {
    ...
    try {
      if (extCanvas.contextContainer) {
        extCanvas.contextContainer = null;
      }
      if (extCanvas.contextTop) {
        extCanvas.contextTop = null;
      }
    } catch (e) {
