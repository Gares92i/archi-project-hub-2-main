...
import { Image, Mail, Trash2, Plus, CheckCircle, MoreHorizontal } from 'lucide-react';
...
export const AnnotationDialog: React.FC<AnnotationDialogProps> = ({
  ...
  const [isConvertToTaskDialogOpen, setIsConvertToTaskDialogOpen] = useState(false);
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={() => setIsOpen(false)}>
        <DialogContent className={cn("sm:max-w-md", isMobile && "p-4 w-[calc(100vw-32px)] max-h-[80vh] overflow-y-auto")}>
          <DialogHeader className="flex-row items-center justify-between space-y-0 pb-2">
            <div className="flex items-center gap-2">
              <DialogTitle className="text-lg">{selectedAnnotation ? `${selectedAnnotation.id} commentaire` : ''}</DialogTitle>
              <DialogDescription>
            ...
            <div className="flex items-center space-x-2">
              ...
              <Button
                ...
                title={selectedAnnotation?.isResolved ? "Marquer comme non résolu" : "Marquer comme résolu"}
              >
                <CheckCircle className={cn("h-4 w-4", selectedAnnotation?.isResolved ? "text-green-500" : "text-muted-foreground")} />
              </Button>
          ...
          <div className="space-y-4 py-4">
            <div className="flex items-start gap-3">
              <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center text-xs font-medium">
                {selectedAnnotation?.author.split(" ").map(word => word[0]).join("").toUpperCase()}
              </div>
              <div className="flex-1">
                <p className="text-sm">{selectedAnnotation?.comment}</p>
                <p className="text-xs text-muted-foreground mt-1">{selectedAnnotation?.date}</p>
                
                {selectedAnnotation?.photos && selectedAnnotation.photos.length > 0 && (
                  <div className={cn("grid gap-2 mt-3", isMobile ? "grid-cols-1" : "grid-cols-2")}>
      ...
      <ConvertToTaskDialog 
        ...
        projectId={(selectedAnnotation && selectedAnnotation.projectId) || projectId || "1"}
        onTaskCreated={() => {
          setIsConvertToTaskDialogOpen(false);
          setIsOpen(false);
          if (onConvertToTask && selectedAnnotation) {
            onConvertToTask(selectedAnnotation.id);
          }
        }}
      />
    </>
