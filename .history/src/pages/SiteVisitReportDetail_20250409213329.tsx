...
import { getProjectAnnotations } from "@/components/services/annotationService";
import {
  Card,
  CardContent,
  CardHeader,
...
const SiteVisitReportDetail = () => {
  ...
  const reservesByDocument = React.useMemo(() => {
    const grouped: Record<
      string,
      { document: string; reserves: typeof report.reserves }
    > = {};

    if (report?.reserves) {
      report.reserves.forEach((reserve) => {
        // Rechercher le nom du document à partir d'une source plus fiable
        // Si vous avez accès aux annotations du projet, utilisez-les
        const documentName =
          reserve.annotationId && projectAnnotations
            ? projectAnnotations.find((a) => a.id === reserve.annotationId)
                ?.documentName || reserve.documentName || "Plan du projet"
            : reserve.documentName || "Plan du projet";
  ...
  // Regrouper les annotations par document pour l'affichage
    ...
    projectAnnotations.forEach(annotation => {
      ...
      if (!grouped[documentId]) {
        grouped[documentId] = {
          documentName,
          documentId,
          documentUrl: annotation.documentUrl || `/placeholder-document-${documentId}.jpg`,
          annotations: []
        };
      }
        ...
        <Card className="mb-6 print:shadow-none print:border-none">
          ...
          <CardContent className="pt-6">
            <div className="space-y-6">
              ...
              {/* Section des documents avec annotations */}
              {shouldShowSection("documentsWithAnnotations") &&
                Object.keys(annotationsByDocument).length > 0 && (
                  <div className="mt-8">
                    <h3 className="text-lg font-medium mb-4">
                      Documents avec annotations
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Object.entries(annotationsByDocument).map(([docId, doc]) => (
                        <div
                          key={docId}
                          className="border rounded-md overflow-hidden">
                          <div className="relative">
                            <img
                              src={doc.documentUrl || "/placeholder-document.jpg"}
                              alt={doc.documentName}
                              className="w-full h-48 object-contain bg-slate-100"
                            />
                            <span className="absolute bottom-2 right-2 bg-orange-500 text-white px-2 py-1 text-xs rounded-full">
                          ...
                          <div className="p-3">
                            ...
                            <div className="mt-2 divide-y divide-gray-100">
                              {doc.annotations.slice(0, 2).map((annotation, idx) => (
                                <p key={idx} className="text-xs text-muted-foreground py-1">
                                  {annotation.comment || "Annotation sans commentaire"}
                                </p>
                              ))}
                              {doc.annotations.length > 2 && (
                                <p className="text-xs text-muted-foreground text-center pt-1">
                                  + {doc.annotations.length - 2} autres annotations
                                </p>
                              )}
                            </div>
                          </div>
