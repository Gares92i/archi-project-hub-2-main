...
import { FileText, ChevronDown, FolderOpen, Plus } from "lucide-react";
...
export const DocumentsSidebar: React.FC<DocumentsSidebarProps> = ({
  ...
  useEffect(() => {
    if (prevDocLength.current !== documents.length) {
      console.log("Documents mis à jour dans sidebar:", 
        ...
        <div className="p-2">
          ...
          <div className="mt-2 ml-6">
            {sortedDocuments.length === 0 ? (
              ...
              <>
                <h3 className="mb-1 font-medium text-xs">Documents ({sortedDocuments.length})</h3>
                <ul className="space-y-1">
                  {sortedDocuments.map((doc) => (
                    <li
                      ...
                      <span className="text-xs truncate max-w-[150px]" title={doc.name}>
